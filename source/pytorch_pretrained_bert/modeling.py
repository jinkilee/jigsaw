...
def train_classifier_model(model, train_dataloader, optimizer):
	losses = AverageMeter()

	# transform to training mode
	model.train()
	...
	for step, batch in enumerate(tk0):
		...
		# FIXME: i don't like to use numpy in this file
		loss = model(input_ids, input_mask, labels=labels)
		...
		# update and display loss
		losses.update(loss.data)
		tk0.set_description("Training loss: {:.4f}".format(losses.avg))
...
def validate_classifier_model(model, valid_dataloader, threshold=0.5):
	...
	for step, batch in enumerate(tk0):
		...
		pred = model(input_ids, input_mask, labels=None)
